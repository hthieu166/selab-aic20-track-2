...
def viz_spectrogram(j, samples, sampling_freq, axes, fig,
    ...
    im_handl = axes.pcolormesh(stft_time, stft_freq, stft_resp)
    #axes.legend().set_visible(False)
    #fig.colorbar(im_handl, ax=axes)
    axes.grid(False)    

    # axes.set_title('Log-scale STFT - {}-axis'.format(names[j]))
...
def viz(loader, args):
    ...
    for i, (samples, labels, msr_ids) in enumerate(loader):
        ...
        # UNCOMMENT TO SAVE RAW 
        # np.save(os.path.join(img_dir, msr_ids[0]+'.npy'), samples)
        ...
        print(str(n_samples)+ ' -- ' + str(sampling_freq))
        ...
        # UNCOMMENT TO SAVE FFT
            stft_freq, stft_time, stft_resp = stft(samples[:, j], fs=sampling_freq)
            stft_resp = (np.log(np.abs(stft_resp)))  # log scale of real part
            output = [stft_freq, stft_time, stft_resp]                        
            print(str(stft_freq.shape) + ' -- ' + str(stft_time.shape) + ' -- ' + str(stft_resp.shape))
            #print('time shape: ' + )
            #print('resp shape: ' + )
            #print(stft_freq.tolist()[:10])
            f = open(os.path.join(img_dir, str(msr_ids[0]))+'-'+axis_labels[j-1]+'.pkl', 'wb')
            pickle.dump(output, f)
            f.close()

        # X axis ##############################################################################
        # Plot the figure UNCOMMENT TO PLOT
        # figx, axesx = plt.subplots(1, 1, figsize=(20, 5), constrained_layout=True)
        ...
        # UNCOMMENT TO VIZ:
        #viz_spectrogram(1, samples, sampling_freq, axesx, figx,
        #                args.min_band, args.max_band)
        #figx.suptitle(parse_labels(labels, args.lbl_type))

        # Save the figure UNCOMMENT TO SAVE
        #img_fname = os.path.join(img_dir, msr_ids[0]+'_x.png')
        #figx.savefig(img_fname, bbox_inches=0, pad_inches = 0)
        #plt.close(figx)

        # Y axis ##############################################################################

        #figy, axesy = plt.subplots(1, 1, figsize=(20, 5), constrained_layout=True)
        ...
        # viz_spectrogram(samples, sampling_freq, axes[2:5], fig,
        #                 args.min_band, args.max_band)
        #viz_spectrogram(2, samples, sampling_freq, axesy, figy,
        #                args.min_band, args.max_band)
        #figy.suptitle(parse_labels(labels, args.lbl_type))

        # Save the figure
        #img_fname = os.path.join(img_dir, msr_ids[0]+'_y.png')
        #figy.savefig(img_fname,bbox_inches=0, pad_inches = 0)
        #plt.close(figy)

        # Z axis ##############################################################################

        #figz, axesz = plt.subplots(1, 1, figsize=(20, 5), constrained_layout=True)
        ...
        # viz_spectrogram(samples, sampling_freq, axes[2:5], fig,
        #                 args.min_band, args.max_band)
        #viz_spectrogram(3, samples, sampling_freq, axesz, figz,
        #                args.min_band, args.max_band)
        #figz.suptitle(parse_labels(labels, args.lbl_type))

        # Save the figure
        #img_fname = os.path.join(img_dir, msr_ids[0]+'_z.png')
        #figz.savefig(img_fname, bbox_inches=0, pad_inches = 0)
        #plt.close(figz)
